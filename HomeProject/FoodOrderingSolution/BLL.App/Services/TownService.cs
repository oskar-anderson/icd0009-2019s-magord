...
{
    public class TownService : BaseEntityService<ITownRepository, IAppUnitOfWork, DAL.App.DTO.Town, BLL.App.DTO.Town>,
        ITownService
    {
        public TownService(IAppUnitOfWork unitOfWork) 
            : base(unitOfWork, new BaseBLLMapper<DAL.App.DTO.Town, BLL.App.DTO.Town>(), unitOfWork.Towns)
        {
        }
        
        public async Task<IEnumerable<BLL.App.DTO.Town>> AllAsync(Guid? userId = null) =>
            (await ServiceRepository.AllAsync(userId)).Select( dalEntity => Mapper.Map(dalEntity) );

        public async Task<BLL.App.DTO.Town> FirstOrDefaultAsync(Guid id, Guid? userId = null) =>
